...
//orders pending
//orders pending
$result_cart_price = mysqli_query($con, $cart_query_price); // Fetch products again from cart
while ($row_pending_order = mysqli_fetch_array($result_cart_price)) {
    $product_id = $row_pending_order['product_id'];
    $quantity = $row_pending_order['quantity'];
    if ($quantity == 0) {
        $quantity = 1;
    }
    ...
    if (!$result_pending_orders) {
        echo "<script>alert('Error inserting pending orders for product ID: $product_id');</script>";
    }
}
